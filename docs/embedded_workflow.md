...
- you have prepared a Simulink model with the name `'dSPACE_Model_Name'.slx`, which is correctly preconfigured and you were able to compile it for your dSPACE DS1202 (MicroLabBox) Platform.
...
On your system in `C:/ProgramData/dSPACE/`, find the folder which contains the directories `/Compiler/QNX650_520/host/win32/x86/` and `/Compiler/QNX650_520/target/qnx6/`, and update the two paths in the toolchain file with this folder.
...
4. In order to install the cross-compiled `acados` version, run:
 ```cmake --install ./```
If all these steps worked, you will find in your new build folder `buildDS1202` a folder `install`, in which you will find the two folders `lib` and `include`.
...
1. Create a new folder `'dSPACE_Build_Folder_Name'` (anywhere) and copy your Simulink model `'dSPACE_Model_Name'.slx`, the dSPACE Makefile `'dSPACE_Model_Name'_usr.mk` and the `acados` S-Function folder `c_generated_code` into this new folder.
...
These files are listed as `SOURCES` in the Matlab script `make_sfun.m` (and `make_sfun_sim.m` if simulation S-Function is used too).
Open the dSPACE Makefile `'dSPACE_Model_Name'_usr.mk` and list all source files needed for the S-Functions, except for the source files which have the same name as the S-Functions.
...
In order to compile your dSPACE Simulnk model `'dSPACE_Model_Name'.slx` use the `rtwbuild` command or press Ctrl+B in Simulink.
The Makefile should now integrate all needed files for the compilation of the `acados` S-Functions.
